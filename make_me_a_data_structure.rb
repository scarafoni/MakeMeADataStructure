...
if __FILE__ == $PROGRAM_NAME
  ...
  if order
    ...
    file_text << "end \n\n"
    # hashtable class
    file_text << "class HashTable\n"
    file_text << "  def initialize()\n"
    file_text << "    @table = Array.new(1000)\n"
    file_text << "  end\n"
    file_text << "  \n"
    file_text << "  def add(new_node)\n"
    file_text << "    table \n"
    file_text << "    @table[int(new_node) % 1000] = newNode\n"
    file_text << "  end\n"
    file_text << "  def get(node)\n"
    file_text << "    return @table[int(node) % 1000]\n"
    file_text << "  end\n"
    file_text << "end\n\n"
  end
  # now write it to the file
  ds_file.write(file_text)
  ds_file.close
end
